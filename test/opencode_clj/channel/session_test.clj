(ns opencode-clj.channel.session-test
...
(deftest create-session-with-options-test
  (let [store (session/create-store)
        sess (session/create-session store "rabbitmq"
                                     {:id "test-session-123"
                                      :routing-key "opencode.session.test"
                                      :context {:user "alice"}})]
    (is (= 1 (session/session-count store)))
    (is (= "test-session-123" (:id sess)))
    (is (= "opencode.session.test" (:routing-key sess)))
...
(deftest get-session-by-routing-key-test
  (let [store (session/create-store)
        sess (session/create-session store "rabbitmq"
                                     {:routing-key "opencode.session.user-alice"})]
    (is (some? (session/get-session-by-routing-key store "opencode.session.user-alice")))
