(ns opencode-clj.channel.channel-test
...
(deftest extract-session-id-test
  (is (= "abc123" (ch/extract-session-id "opencode.session.abc123")))
  (is (= "user-alice" (ch/extract-session-id "opencode.session.user-alice")))
  (is (nil? (ch/extract-session-id "opencode.user.alice")))
  (is (nil? (ch/extract-session-id "opencode.broadcast"))))

(deftest extract-user-id-test
  (is (= "alice" (ch/extract-user-id "opencode.user.alice")))
  (is (= "bob@example.com" (ch/extract-user-id "opencode.user.bob@example.com")))
  (is (nil? (ch/extract-user-id "opencode.session.abc123"))))

(deftest make-session-routing-key-test
  (is (= "opencode.session.abc123" (ch/make-session-routing-key "abc123")))
  (is (= "opencode.session.user-alice" (ch/make-session-routing-key "user-alice"))))

(deftest make-user-routing-key-test
  (is (= "opencode.user.alice" (ch/make-user-routing-key "alice")))
  (is (= "opencode.user.bob@example.com" (ch/make-user-routing-key "bob@example.com"))))

(deftest make-channel-routing-key-test
  (is (= "opencode.channel.cli" (ch/make-channel-routing-key "cli")))
  (is (= "opencode.channel.rabbitmq" (ch/make-channel-routing-key "rabbitmq"))))

(deftest broadcast-routing-key-test
  (is (= "opencode.broadcast" ch/broadcast-routing-key)))
...
(deftest cli-send-message-test
  (let [cli-ch (cli/create-cli-channel {})]
    ...
    (let [result (ch/send-message cli-ch "opencode.session.test" "Hello World!" {})]
