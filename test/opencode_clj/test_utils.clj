(ns opencode-clj.test-utils
  "Test utilities for opencode-clj tests.

   Provides helpers for testing against a real opencode-server
   running at http://127.0.0.1:9711

   Usage:
   (require '[opencode-clj.test-utils :as tu])
...
(defn with-clean-session
  ...
  [client sym & body]
  `(let [~sym (let [resp# (opencode-clj.sessions/create-session ~client)]
                (when-let [id# (:id resp#)]
                  id#))]
     (try
       ~@body
       (finally
         (when ~sym
           (try
             (opencode-clj.sessions/delete-session ~client ~sym)
             (catch Exception _#)))))))

(defmacro with-session
  "Create a session for testing, automatically clean up after."
  [client-sym session-sym & body]
  `(let [~session-sym (opencode-clj.sessions/create-session ~client-sym)]
     (try
       ...
       (finally
         (when-let [id# (:id ~session-sym)]
           (try
             (opencode-clj.sessions/delete-session ~client-sym id#)
             (catch Exception _#)))))))
