(ns opencode-clj.channel.rabbitmq
    ...
    (if @(:running? this)
      ...
      (try
        (let [uri (get-in (:config this) [:uri] "amqp://guest:guest@localhost:5672")
              ...
              ch (lch/open conn)
              ex-name (get-in (:config this) [:exchange] "opencode.messages")
              q-name (get-in (:config this) [:queue] "opencode.inbox")
              rk (get-in (:config this) [:routing-key] "opencode")
              ;; Declare exchange
...
(defn create-rabbitmq-channel
  "Create a new RabbitMQ channel.
   ...
   opts:
     ...
     :exchange        - Exchange name (default: opencode.messages)
     :queue           - Queue name (default: opencode.inbox)
