...
(defrecord CliChannel
  ...
  (start [this]
    (when-not @(:running? this)
      ...
      (async/thread
        (run-input-loop this))
      (println (str "\n" (:prompt this "opencode> ") "Ready. Type 'help' for commands, 'exit' to quit."))
      (println (str (:prompt this "opencode> ")))
      (flush))
    this)
...
(defn create-cli-channel
  "Create a new CLI channel.
   ...
   Options:
     ...
     :bus                  - Bus instance for message routing
     :prompt               - Prompt string (default: 'opencode> ')"
