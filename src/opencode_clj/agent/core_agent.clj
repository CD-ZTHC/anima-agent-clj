(ns opencode-clj.agent.core-agent
  ...
  (:require
   ...
   [opencode-clj.bus.core :as bus-core]
   [opencode-clj.agent.worker-agent :as worker]
   [opencode-clj.agent.worker-pool :as pool]
   ...
   [opencode-clj.context.manager :as context-manager]
   [opencode-clj.cache.lru :as cache-lru]
   [opencode-clj.cache.core :as cache-core]
  ...
  "Create a new CoreAgent.
   ...
   Options:
     ...
     :metrics-config   - Metrics configuration (Phase 3)
       :prefix         - Metric prefix (default: \"opencode\")"
