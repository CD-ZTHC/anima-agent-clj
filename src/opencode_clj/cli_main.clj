...
(defn start-cli
  "Start the full CLI architecture.
   ...
   Options:
     ...
     :prompt - CLI prompt string (default: 'opencode> ')"
  [{:keys [url prompt]
    :or {url "http://127.0.0.1:9711"
         prompt "opencode> "}}]
  (println "╔══════════════════════════════════════════╗")
  (println "║       OpenCode CLI (opencode-clj)        ║")
  (println "╚══════════════════════════════════════════╝")
  (println)
  (println "  Server:" url)
  ...
  (let [opts (parse-args args)]
    (if (:help opts)
      (do
        ...
        (println "  --prompt <prompt>  CLI prompt string (default: 'opencode> ')")
